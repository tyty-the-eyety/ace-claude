...
- `ace-claude/examples/unittest/` — runtime-proven headless flow unit testing (Test Project + NodeSpy + **NodeStub** + ibmint testzip + IntegrationServer --test-project); read its `README.md` before writing flow tests — a stubbed flow MUST be driven from its input node with `propagate()` (calling `evaluate()` on the stub silently runs the REAL node), `evaluate()`'s boolean means "in isolation" not "propagate", and read paths are dotted-from-Root (`JSON.Data.x`) while `ignorePath` is slash-prefixed (`/JSON/Data/x`)
