...
- `ace-claude/examples/unittest/` — worked test classes (NodeSpy, NodeStub, async pairs, self-deploy). **For testing work read `ace-claude/Testing.md` first** — it holds the harness model and all the rules; the examples README only indexes the files
...
- `ace-claude/examples/timer/` — runtime-proven Scheduler (interval + cron), automatic TimeoutNotification, and the TimeoutControl set/cancel + controlled-replay pair; read its `README.md` and `ace-claude/Timer.md` BEFORE using any timer node — `scheduleType` is `interval` not `repeatInterval`, the cron dialect is literals-only, a stored message with no `MQMD` is silently lost, and the Timer policy's `queuePrefix` is an INFIX (`SYSTEM.BROKER.TIMEOUT.<prefix>.QUEUE`, auto-created by ACE) that attaches only to the two timeout nodes, never the Scheduler
- `ace-claude/examples/routing/` — runtime-proven Route (HTTP + MQ), Filter, RouteToLabel + Label, and FlowOrder flows; read its `README.md` before using any routing node — a Filter's ESQL is a `FILTER MODULE` using `Root` NOT `InputRoot` (an `InputRoot` there fails at deploy and stops the whole application), and RouteToLabel reads an undocumented `LocalEnvironment.Destination.RouterList.DestinationData[n].labelName` list
- `ace-claude/examples/rest/` — runtime-proven REST Request and RESTAsyncRequest/Response flows against the Swagger Petstore; read its `README.md` and `ace-claude/REST.md` before using any REST node — they are **spec-driven, not URL-driven** (`definitionFile` + `operationName` mandatory, no URL in the flow), parameters are a repeated child element `<parameters name= type= expression=/>` and NOT the XSD's nested row form, and `RESTAsyncResponse` is an input node paired by correlator so a correct async pair looks disconnected
- `ace-claude/examples/slack/` — runtime-proven Slack connector flows (private-channel retrieve + send message); read its `README.md` and `ace-claude/Slack.md` before ANY connector work, not just Slack — three of its findings are connector-wide: `OutTerminal.noData` must be wired on every retrieve or an empty result set becomes a 404, a `CREATE` needs its `gen/*.request.schema.json` to exist, and `OBJECT_NAME` is a mandatory parent discriminator in the request body
...
Read `ace-claude/Testing.md` when — and only when — the task involves flow unit tests: writing or debugging a Test Project, a JUnit test class, `testproject.descriptor`, or a test classpath. It is NOT needed for authoring msgflow/ESQL/Java/map artifacts, so do not load it for ordinary flow work.
...
- Do not set `computeMode` on a Compute node unless the ESQL touches `OutputLocalEnvironment` or `OutputDestination` — and when it does, the value is `destinationAndMessage` (or `destination`): there is no `localEnvironment*` literal, and an invalid one is accepted silently by both `ibmint` and the runtime, which then drops the LocalEnvironment
...
- Validate with `mqsicreatebar -data <ws> -a <APP> -cleanBuild` as well as `ibmint package`; read the verdict from the `Problem N: Resource - /PROJECT/...` lines, not the log tail, and note no BAR is written if ANY project in the workspace has errors — when unrelated projects are already broken, copy the one project into an empty directory and run `mqsicreatebar -data <thatdir>` against it to get a clean verdict
- **Add `-deployAsSource` to `mqsicreatebar` for any app containing connector nodes** (`com_ibm_connector_*`, `ComIbmApplicationConnector*`). They cannot be compiled to CMF, and without the flag the build reports ZERO `Problem N:` lines yet writes a BAR holding only `META-INF` — a silent failure. The real message is buried in the log: "cannot be added as CMF. You cannot use the compile and in-line resource option". Always confirm the BAR actually contains `<APP>.appzip`
